...
function getDiskUsage() {
  try {
    ...
    if (process.platform === 'win32') {
      try {
        const output = execSync('wmic logicaldisk where DeviceID="C:" get Size,FreeSpace', { encoding: 'utf8', timeout: 3000 });
        const lines = output.trim().split('\n');
        if (lines.length > 1) {
          const parts = lines[1].trim().split(/\s+/);
          if (parts.length >= 2) {
            total = parseFloat(parts[1]) || 0;
            free = parseFloat(parts[0]) || 0;
            used = total - free;
          }
        }
      } catch (e) {
        const output = execSync('powershell -Command "Get-PSDrive -Name C | Select-Object @{N=\'Used\';E={$_.Used}}, @{N=\'Free\';E={$_.Free}}"', { encoding: 'utf8', timeout: 3000 });
        const lines = output.trim().split('\n').filter(l => l.trim());
        if (lines.length > 0) {
          const lastLine = lines[lines.length-1].trim().split(/\s+/);
          if (lastLine.length >= 2) {
            used = parseFloat(lastLine[0]) || 0;
            free = parseFloat(lastLine[1]) || 0;
            total = used + free;
          }
        }
      }
      ...
      try {
        const output = execSync("df -k / | tail -1", { encoding: 'utf8', timeout: 3000 });
        const parts = output.trim().split(/\s+/);
        if (parts.length >= 4) {
          total = parseFloat(parts[1]) * 1024;
          used = parseFloat(parts[2]) * 1024;
          free = parseFloat(parts[3]) * 1024;
        }
      } catch (e) {
        throw e;
      }
    }
    if (total === 0) throw new Error('无法获取磁盘信息');
    return {
    ...
    adminLog('获取磁盘信息失败，使用内存信息替代: ' + err.message, 'WARN');
    const mem = os.totalmem();
    const freeMem = os.freemem();
    return {
      totalGB: (mem / 1024 / 1024 / 1024).toFixed(1),
      usedGB: ((mem - freeMem) / 1024 / 1024 / 1024).toFixed(1),
      usedPercent: Math.round(((mem - freeMem) / mem) * 100)
    };
  }
}
